...
def _pandas_insert():
    ...
    data_set = pandas.read_excel(
        os.path.join(settings.DATA_DIRECTORY, "clearing_201810_201910.xlsx"),
        usecols="A:M",
        sheet_name=1,
        skiprows=9,
        nrows=100000,
    ...
    data_set = [dict(zip(column_name, tuple(x))) for x in data_set.to_numpy()]
    logger.info(f"Parsed {len(data_set)}")

    database.insert_bulk_data(dataset=data_set, tablename="settlment")
...
def main():
    ...
    for table_name, file_name in DATA_MAPPING.items():
        ...
        database.insert_bulk_data(dataset=formatted_rows, tablename=table_name)
