...
def get_engine(endpoint_spec: Dict[str, str] = PARAMETER_DICT) -> Engine:
    engine = create_engine(
        CONNECTION_FORMAT.format(**endpoint_spec), pool_pre_ping=True
    )
...
def get_table(engine: Engine, tablename: str, schema: str = None) -> Table:
...
def insert_bulk_data(dataset: List[Dict[str, Any]], tablename: str = None):
    try:
        ...
        for chunk in get_chunk(dataset, chunk_size=2000):
            ...
            session.bulk_insert_mappings(meta_table, chunk)
