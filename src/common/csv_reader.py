...
def _strip_out_headers(
    ...
    reader = csv.DictReader(
        read_from, delimiter=delimiter, fieldnames=None, dialect=csv.excel,
    )

    fieldnames = [_set_encoding(colname.strip()) for colname in reader.fieldnames]

    return fieldnames, newline_delimiter.join(body.split(newline_delimiter)[1:])
...
def parse_csv(body: bytes) -> List[Dict[str, Any]]:
    ...
    delimiter = _get_delimiter(sample_data)

    newline_delimiter = _sniff_newline(body=body[1:2000])

    headers, raw_body = _strip_out_headers(
        body=body, newline_delimiter=newline_delimiter, delimiter=delimiter
    )

    read_body = io.StringIO(raw_body, newline_delimiter)
    reader = csv.reader(read_body, delimiter=delimiter, dialect=csv.excel,)

    formatted_rows = [
        dict(zip(headers, _set_encoding_dict(unformatted))) for unformatted in reader
    ]
